...
function renderSummaryCards() {
    ...
    const dokodoCard = grid.querySelector('article[data-dokodo-card="true"]');
    ...
    const summaryCardsHTML = state.summaries.map((summary, index) => {
        ...
        const htmlPath = summary.file.replace('.md', '.html');
        return `
            <a href="${htmlPath}" 
               class="card-hover fade-in rounded-xl shadow-lg p-6 cursor-pointer border block"
               style="background: var(--surface-elevated); border-color: var(--border-primary); text-decoration: none;"
    ...
    // Set innerHTML with summary cards first, then Dokodo card at the end
    grid.innerHTML = summaryCardsHTML + dokodoCardHTML;
...
function renderSearchResults(results) {
    ...
    searchResults.innerHTML = results.map(result => {
        ...
        // Convert markdown file path to HTML file path
        const htmlPath = result.summary.file.replace('.md', '.html');
        return `
            <a href="${htmlPath}" 
               class="p-4 rounded-lg cursor-pointer transition-colors mb-2 block"
